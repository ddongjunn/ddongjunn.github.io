...
Java/Spring Boot 기반 5년차 백엔드 개발자로, 장애 상황에서도 예측 가능하게 동작하는 서버 구조를 중요하게 생각
합니다. 외부 시스템 연동, 동시성 충돌, 장애 전파 문제를 경험하며 기능이 정상 동작하는 것뿐 아니라 실패했을 때 시스
템이 어떻게 반응해야 하는지도 함께 고민해왔습니다. 트랜잭션 정합성, 책임 레이어 분리, 장애 격리처럼 운영 환경에서
드러나는 문제를 구조 관점에서 해결하는 데 관심이 많습니다.
...
- Ceph 클러스터 장애 전파 차단 아키텍처 재설계로 **P95 레이턴시 10.03s → 78ms** 개선
- HAProxy/Keepalived 기반 단일 진입점, Resilience4j 기반 Provider별 CircuitBreaker 격리 구조 설계
- Ceph Exporter, Node Exporter, Micrometer 기반 커스텀 메트릭 수집 및 Slack 장애 알림 구성
- go-ceph Custom Container 기반 확장 API로 이미지 분석 **29.4s → 0.9s** 개선
...
- 슬로우 쿼리 분석 및 인덱스 설계로 API 응답 시간 **3.375s → 0.016s** 개선
- Scouter APM 도입으로 API 요청 구간별 및 SQL 단위 병목 분석 체계 구축
- AOP 기반 ERP 공통 인증 모듈로 API별 인증 중복 제거 및 결재 정합성 개선
- 트랜잭션 커밋 이후 비동기 이벤트로 메일 발송을 분리해 외부 시스템 실패 격리
...
k6 기반 장애 시나리오 테스트와 재측정을 통해 기존 redirect/fallback 구조가 장애 상황에서 응답 지연을 증폭시키는 원인임을 확인했습니다. 또한 VIP 기반 단일 진입점 호출 구조와 비교한 결과, 문제의 본질이 재시도 로직이 아니라 Active MGR 선택과 failover 책임 배치에 있음을 확인했습니다.

#### 해결

- Single-flight 패턴을 적용해 동일 Provider 대상 토큰 중복 발급과 DB Lock 경합 완화
...
[Ceph 클러스터 장애 전파를 차단한 서버 고가용성 구조 개선](/posts/cluster-failure-isolation/)
...
단순 설치 자동화만으로는 반복적인 클러스터 구성 변경에 대응하기 어렵다고 판단했습니다. 클러스터 설정, 서비스 배포, 리소스 구성을 코드로 표준화하고, 노드 구성·네트워크·서비스 옵션을 환경별로 변경할 수 있도록 변수·템플릿 기반의 Ansible 자동화 구조를 설계했습니다.
...
인증 로직은 API별 개별 구현보다 공통 모듈로 분리하는 것이 유지보수와 확장성 측면에서 적합하다고 판단했습니다. 또한 ERP 응답 결과를 기준으로 결재 승인 흐름을 제어해 기존 프로세스 변경 범위를 최소화하는 방향으로 접근했습니다.
...
_코비젼 · 2022.06 ~ 2022.08 · Java, Spring, MyBatis, MySQL_

#### 문제

동일 장비에 대한 동시 예약 요청 시 이중 예약이 발생할 수 있었고, 예약 성공 이후 외부 ERP 시스템으로 예약 정보를 연동해야 하는 구조였습니다.

#### 분석

예약 도메인에서는 충돌 빈도보다 정합성 보장이 중요하다고 판단했습니다. 요청 전체를 직렬화하기보다 충돌 시점에만 검증하는 방식이 적합하다고 보고, MyBatis 환경에서 version 기반 낙관적 락을 직접 구현했습니다.

#### 해결

- version 조건부 UPDATE와 affected row 검증으로 예약 성공 조건을 원자적으로 검증
- 충돌 발생 시 예약 실패 처리 및 사용자 예외 응답 반환
- 예약 성공 이후 외부 ERP 시스템으로 예약 정보 연동

#### 성과

- 동일 장비에 대한 이중 예약 가능성 차단
- 요청 직렬화 없이 동시 예약 요청에 대한 정합성 보장
