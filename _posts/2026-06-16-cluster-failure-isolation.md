...
- Ceph 클러스터 장애 시 단건 호출 기준으로 응답 시간이 1.15초까지 증가했고, 부하 테스트에서는 요청이 밀리며 평균 5.04초까지 늘어나 지연이 발생했습니다.
...
└─ 결과 반환
```
이를 통해 토큰 발급 중복 호출을 줄이고, Active MGR 갱신 이벤트 중복 발생과 DB Optimistic Lock 충돌을 완화했습니다. 결과적으로 이후 구조 개선 효과를 신뢰할 수 있는 테스트 환경을 만들 수 있었습니다.
...
HAProxy의 HTTP Health Check는 200 응답만 정상으로 판정하도록 구성했습니다. Active MGR만 200을 반환하므로, 303을 반환하는 Standby MGR와 응답하지 않는 Down MGR를 라우팅 대상에서 제외했습니다.
...
| dropped_iterations | 989 | 11 | iteration 누락 감소 |
| 동시 VUs | 50 (max) | 16 | 동일 부하를 1/3 수준의 VU로 처리 |
| 장애 클러스터 fast-fail | 불가 | 98~99% (100ms 이내) | 장애 즉시 실패 반환 |
| 장애 감지 | 수동 | 메트릭 + 알림 자동 감지 | 모니터링 확보 |
_fast-fail 비율은 장애 클러스터 요청 중 5xx이면서 100ms 이내에 응답한 비율입니다._
